...
//	private static SimpleDateFormat	minSdf		= new SimpleDateFormat("HH:mm:ss");
    ...
    public static String dateToDayStr(long time)
    {
        return dateToDayStr(new Date(time));
    }
     ...
     * @return
     */
     public static Long getTodayTimeSS(String time) {
...
//			System.out.println("Format To times:"+date.getTime());
     ...
     * 防审核
     */
    public static boolean fshDateTime(int allGames, double allRecharge, long loginTime) {
        if (allGames == 0) {
            return false;
        }
        if (allRecharge == 0) {
        ...
        if (getTodayTime("00:00") < loginTime && getTodayTime("06:00") > loginTime) {
            return true;
        }
        if (getTodayTime("19:00") < loginTime && getTodayTime("23:59") > loginTime) {
            return true;
        }
