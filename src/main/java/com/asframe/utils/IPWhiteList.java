...
public class IPWhiteList {
    ...
    public static boolean checkLoginIP(String ip, String ipWhiteConfig) {
        if (StringUtils.isBlank(ip) || StringUtils.isBlank(ipWhiteConfig)) {
            return false;
        }
    ...
    /**
     * checkLoginAccount:(根据账户，及账户白名单设置规则判断账户是否包含在白名单).
     *
     * @return
     */
    public static boolean checkLoginAccount(String account, String accountWhiteConfig) {
        String[] list = accountWhiteConfig.split(",");
        for (int i = 0; i < list.length; i++) {
            if (StringUtils.isNotBlank(list[i])) {
                if (list[i].equals(account)) {
                    return true;
                }
            }
        }
